...
async function testUploadAndPipeline() {
  ...
  const pdfExtracted = await extractDocumentText({ buffer: pdfBuffer, filename: 'sample_resume.pdf' });
  console.log(`✓ PDF extracted successfully: ${pdfExtracted.charCount} chars, ${pdfExtracted.pageCount} page(s)`);
  ...
  const docxExtracted = await extractDocumentText({ buffer: docxBuffer, filename: 'sample_resume.docx' });
  ...
  const txtExtracted = await extractDocumentText({ buffer: txtBuffer, filename: 'sample_resume.txt' });
